  ...
  <v-layout wrap>
    ...
    <v-flex xs12 sm3 mb-2>
      <v-layout align-center justify-end v-if="projectInfo.stage">
        ...
        >{{ $t('common.fields.report') }} {{ reportNum }}</v-chip>
      </v-layout>
    </v-flex>
    ...
    >
      ...
      <v-card class="mb-4 project-details-info">
        <v-list dense>
          ...
          <v-list-tile v-if="faceReportData.createdAt">
            <v-list-tile-content>{{ $t('common.fields.appliedAt') }}:</v-list-tile-content>
            <v-list-tile-content class="align-end">{{ faceReportData.createdAt }}</v-list-tile-content>
          </v-list-tile>
          <v-list-tile v-if="faceReportData.approvedAt">
            <v-list-tile-content>{{ $t('common.fields.submittedAt') }}:</v-list-tile-content>
            <v-list-tile-content class="align-end">{{ faceReportData.approvedAt }}</v-list-tile-content>
          </v-list-tile>
        </v-list>
      </v-card>
    </v-flex>
    <v-flex xs12 md2 :class="{ 'pl-2': $vuetify.breakpoint.smAndUp }">
      <v-layout
        ...
        :key="key"
        v-if="(faceReportData.statusId) && (faceReportData.statusId !== 'waiting' && faceReportData.statusId !== 'confirm' && faceReportData.statusId !== 'reject')"
      >
        <v-chip
...
<script>
  export default {
    ...
    computed: {
      ...
      reportNum() {
        return (this.isFaceReportCreationPath) ? this.projectInfo.stage.num : this.faceReportData.num;
      },
      faceApprovalStages() {
        const stages = [
          {
    ...
    methods: {
      getColor(stageKey) {
        ...
        switch (stageKey) {
          case 'validate':
            if (this.faceReportData.statusId === 'validate') {
              color = 'info';
            } else {
              color = 'success';
            }
            break;
          case 'certify':
            if (this.faceReportData.statusId === 'certify') {
              color = 'info';
            }
            if (this.faceReportData.statusId === 'approve' ||
                this.faceReportData.statusId === 'verify' ||
                this.faceReportData.statusId === 'success') {
              color = 'success';
            }
            break;
          case 'approve':
            ...
            if (this.faceReportData.statusId === 'verify' ||
                this.faceReportData.statusId === 'success') {
              color = 'success';
            }
            break;
          case 'verify':
            ...
            if (this.faceReportData.statusId === 'success') {
              color = 'success';
            }
            break;
          default:
